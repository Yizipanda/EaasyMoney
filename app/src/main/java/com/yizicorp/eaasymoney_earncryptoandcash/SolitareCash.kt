...
class SolitareCash : AppCompatActivity() {

    lateinit var mAdView: AdView
    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        setContentView(R.layout.activity_solitare_cash)

        ads()
        internet()
        val gohom = findViewById<TextView>(R.id.gohom)
        val  donls = findViewById<Button>(R.id.soldom)


        gohom.setOnClickListener {
            val df = Intent(this,Homepage::class.java)
            startActivity(df)
            finishAffinity()
        }

        donls.setOnClickListener {
            val vfr = Intent(android.content.Intent.ACTION_VIEW)
            vfr.data = Uri.parse("https://solitairecash.com/")
            startActivity(vfr)
    ...
    private fun ads() {
        ...
        mAdView = findViewById(R.id.adViewsol)
